...
规则全部来自本插件配套的笔记规范：不同语言之间空一个字宽、与标点之间不空、中文与数字之间不空。两条会误伤专有名词的规则默认关闭：英文↔数字（`GPT4`、`3D`、`v1.2.2`）与数字↔单位。符号那一类走中文排版规范（W3C [中文排版需求 clreq](https://www.w3.org/TR/clreq/)）：**空隙只加在汉字与西文字母、数字之间**（§6.3.3，不多于四分之一汉字宽，行首行尾不加），标点旁边连这个空隙都不加，汉字与标点更是 1:1 方块、无缝隙并列（§1.2）；CSS `text-autospace` 的默认值同样只在中西文之间加空隙，标点要显式写 `punctuation` 才加。
...
| `src/text-pipeline.ts` | 按固定顺序串起各排版步骤：缩进 → 标记 → 聊天记录 → 智能公式 → 公式 → 空格 → 标签 → 板块排序；整条流水线**迭代到不动点**（最多 5 轮，实测 1–3 轮）—— 后面的步骤会改前面步骤看过的文本：`数字 ↔ 单位` 补出的那一格让智能公式在同一轮里才认得出 `5/10mm` 是算式，公式排版把 `$w\approx 0$` 规范成 `$w \approx 0$` 又会改掉板块排序的键。不迭代的话「排版两次」比「排版一次」多改几行，每次保存都要重写文件 |
...
| `src/symbols.ts` | 逐符号空格规则表：`, . ! ? :` / `\|` / `&` / `→` / `^` / `...` / 包裹符号的左右要求，每条标注笔记出处 |
| `src/line-scan.ts` | 共用保护区判定：frontmatter、围栏代码块、缩进代码块、公式、GFM 表格行 |
