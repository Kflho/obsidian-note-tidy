...
| `src/text/pipeline.ts` | 按固定顺序串起各排版步骤：缩进 → 标记 → 聊天记录 → 列表序号 → 标题级别 → 智能公式 → 公式 → 空格 → 标签 → 板块排序；整条流水线**迭代到不动点**（最多 5 轮，实测 1–3 轮）—— 后面的步骤会改前面步骤看过的文本：`数字 ↔ 单位` 补出的那一格让智能公式在同一轮里才认得出 `5/10mm` 是算式，公式排版把 `$w\approx 0$` 规范成 `$w \approx 0$` 又会改掉板块排序的键。不迭代的话「排版两次」比「排版一次」多改几行，每次保存都要重写文件 |
