/**
 * 智能公式测试（正文里的数学符号自动包 `$…$`）
 *
 * 运行：npm test
 *
 * 分五级保证：
 *   1. 期望输出（golden）—— 笔记里的真实写法逐条钉死：`矩阵 A`、`矩阵A`、`n维`、`V(F)`、
 *      整段算式 `x = 0`、希腊字母
 *   2. 变量表 —— 已经写好的公式（`$z$`、`$e^{At}$`）让全篇同名变量都跟上
 *   3. 安全边界 —— 英文散文、长单词、缩写、盘符路径、型号、命名约定、代码块 / 链接 /
 *      标签 / 已有公式内部都不许动
 *   4. 开关 —— 关掉后一个字都不改
 *   5. 严格幂等 —— 包好的公式再跑一次不会被包第二层
 */
import { DEFAULT_TEXT_MATH_OPTIONS, wrapPlainMath } from "../src/text/math-wrap";

// -------------------------------------------------------------------- 断言
let checks = 0;
const failures: string[] = [];

function show(s: string): string {
	return JSON.stringify(s);
}

function check(name: string, actual: string, expected: string): void {
	checks++;
	if (actual !== expected) {
		failures.push(`[期望输出不符] ${name}\n  期望 ${show(expected)}\n  实际 ${show(actual)}`);
	}
}

function checkTrue(name: string, condition: boolean, detail: string): void {
	checks++;
	if (!condition) failures.push(`[断言失败] ${name}\n${detail}`);
}

/** 一次检查同时覆盖"结果正确"与"幂等" */
function caseCheck(name: string, input: string, expected: string): void {
	const once = wrapPlainMath(input, DEFAULT_TEXT_MATH_OPTIONS);
	check(name, once, expected);
	check(`${name}（再跑一次不变）`, wrapPlainMath(once, DEFAULT_TEXT_MATH_OPTIONS), once);
}

/** 期望"原样不动"的用例 */
function keepCheck(name: string, input: string): void {
	caseCheck(name, input, input);
}

// ------------------------------------------------------------ 1. 该包的
function goldenTests(): void {
	// 单个字母与中文相邻
	caseCheck("矩阵 A：中文两侧", "矩阵 A 中最高阶非零子式的阶数，称为 A 的秩", "矩阵 $A$ 中最高阶非零子式的阶数，称为 $A$ 的秩");
	caseCheck("矩阵A：没有空格", "设矩阵A可逆", "设矩阵$A$可逆");
	caseCheck("量词：n维 / n 阶 / n 次", "在n维空间里，n 阶方阵要乘 n 次", "在$n$维空间里，$n$ 阶方阵要乘 $n$ 次");
	caseCheck("多个单字母：各自成块", "向量 x 与向量 y 垂直", "向量 $x$ 与向量 $y$ 垂直");
	caseCheck("向量组：字母夹在中文里", "若向量组 a 线性无关，且可由向量组 b 线性表示", "若向量组 $a$ 线性无关，且可由向量组 $b$ 线性表示");
	caseCheck("中文标点也能当锚点", "记为 V(F), 也称 V(F) 为 n 维线性空间", "记为 $V(F)$, 也称 $V(F)$ 为 $n$ 维线性空间");
	caseCheck("没有语境的单字母不动", "则 a 为 x 在基下的坐标", "则 a 为 x 在基下的坐标");

	// 字母 + 括号参数
	caseCheck("a(b)：用户给的例子", "有 a(b) 成立", "有 $a(b)$ 成立");
	caseCheck("V(F)：线性空间", "线性空间 V(F) 中的向量组称为 V(F) 的一个基", "线性空间 $V(F)$ 中的向量组称为 $V(F)$ 的一个基");
	caseCheck("T(x)：函数调用", "存在 T(x) 使得 T(x) = 0", "存在 $T(x)$ 使得 $T(x) = 0$");

	// 整段算式一起包（不能只包字母，留下 = 0 在公式外面）
	caseCheck("算式：x = 0", "且 x = 0时，恒有 V(x) = 0，对于", "且 $x = 0$时，恒有 $V(x) = 0$，对于");
	// 包出的 `$x = Tz$` 顺手把 T、z 记进变量表，后文单写的 z 因此也跟上（原来不动）
	caseCheck("算式：x = Tz", "1. x = Tz：x 为原状态，z 为新状态", "1. $x = Tz$：$x$ 为原状态，$z$ 为新状态");
	caseCheck("算式：多字符变量乘积与希腊字母", "有关系 Ax = λx 成立", "有关系 $Ax = \\lambda x$ 成立");
	caseCheck("算式：带数字", "取 k = 3 时成立", "取 $k = 3$ 时成立");
	caseCheck("算式：比较符号", "若 x > 0 则正定", "若 $x > 0$ 则正定");
	caseCheck("算式：减号两侧各留一格", "取 x - 1 的值，最大到 n - 1", "取 $x - 1$ 的值，最大到 $n - 1$");
	caseCheck("算式：减号与等号混用", "在 x - 1 = 0 时恒成立", "在 $x - 1 = 0$ 时恒成立");
	caseCheck("算式：小数", "误差 x = 1.5 之内", "误差 $x = 1.5$ 之内");
	caseCheck("算式：逗号分隔", "设 a, b ∈ F 都为非零元", "设 $a, b \\in F$ 都为非零元");
	caseCheck("算式：中文标点在段的中间", "记为 V(F), 也称 V(F) 为 n 维线性空间", "记为 $V(F)$, 也称 $V(F)$ 为 $n$ 维线性空间");

	// 希腊字母 → LaTeX 命令
	caseCheck("希腊字母：小写", "特征值 λ 对应特征向量", "特征值 $\\lambda$ 对应特征向量");
	caseCheck("希腊字母：大写", "对于 Λ 来说", "对于 $\\Lambda$ 来说");
	caseCheck("希腊字母：算式里也换", "有 ε > 0 存在", "有 $\\epsilon > 0$ 存在");
}

// ------------------------------------------------------- 2. 变量表（已有公式）
/**
 * 你在笔记里把 `z` 写成 `$z$`，就等于声明"z 是变量"：正文后面单写的 `z`
 * 照旧包上，不用再靠语境词猜一遍。跨行、跨段落、`$$…$$` 区块都算数。
 */
function variableTableTests(): void {
	caseCheck(
		"已有行内公式：后文同名变量跟上",
		"设 $z$ 为复数。\n讨论 z 的模长与 Z 的模长",
		"设 $z$ 为复数。\n讨论 $z$ 的模长与 Z 的模长"      // 大小写是两个符号，Z 不跟
	);
	caseCheck(
		"已有公式在别的段落里也管用",
		["设 $z$ 为复数。", "", "第一段里 z 与 Z 的关系：", "- z 的共轭记作 $\\bar z$", "- Z 是矩阵"].join("\n"),
		["设 $z$ 为复数。", "", "第一段里 $z$ 与 Z 的关系：", "- $z$ 的共轭记作 $\\bar z$", "- Z 是矩阵"].join("\n")
	);
	caseCheck(
		"$$ 区块里的变量一样算数",
		["$$z = a + bi$$", "因此 z 的实部与 a 的实部"].join("\n"),
		["$$z = a + bi$$", "因此 $z$ 的实部与 $a$ 的实部"].join("\n")
	);
	caseCheck(
		"跨行 $$ 区块里的变量",
		["$$", "z = a + bi", "$$", "因此 z 的实部"].join("\n"),
		["$$", "z = a + bi", "$$", "因此 $z$ 的实部"].join("\n")
	);
	caseCheck(
		"复合公式里的字母都算变量：$e^{At}$ → e / A / t",
		"$e^{At}$ 是矩阵指数，其中 A 是矩阵，t 是时间，e 是常数",
		"$e^{At}$ 是矩阵指数，其中 $A$ 是矩阵，$t$ 是时间，$e$ 是常数"
	);
	caseCheck(
		"本轮包出来的变量，后文也照旧（跨行）",
		["参数 A 满足条件", "因此 A 可逆"].join("\n"),
		["参数 $A$ 满足条件", "因此 $A$ 可逆"].join("\n")
	);
	caseCheck(
		"公式里的命令名与文字不算变量",
		"$\\sin x$ 的函数，x 是变量，s 与 n 与 m 不是",
		"$\\sin x$ 的函数，$x$ 是变量，s 与 n 与 m 不是"
	);
	caseCheck(
		"`\\text{…}` `\\mathrm{…}` 里的字母不算变量",
		"$\\text{max}$ 与 $\\mathrm{d}x$ 的写法：x 是变量，m 与 a 与 d 不是",
		"$\\text{max}$ 与 $\\mathrm{d}x$ 的写法：$x$ 是变量，m 与 a 与 d 不是"
	);

	// 变量表也不能越界：没有中文锚点、或 `_` `^` 这类连接符旁边，照样不动
	keepCheck("没有中文锚点时仍然不动", ["$z$ 是复数", "z"].join("\n"));
	keepCheck("英文句子里的变量仍然不动", "$z$ 是复数，而 the value z is unknown");
	keepCheck("命名约定仍然不动", "$z$ 是复数，记作 Q_inv 与 z^2 与 z_1");
	keepCheck("代码块里的公式不喂变量表", ["```", "$z$ 是变量", "```", "正文里的 z 不动"].join("\n"));
	keepCheck("行内代码里的 $z$ 不喂变量表", "用 `$z$` 表示，而 z 不动");
	keepCheck("frontmatter 里的 $z$ 不喂变量表", ["---", "title: $z$ 是变量", "---", "正文里的 z 不动"].join("\n"));
	keepCheck("没配成对的 $ 不算公式", "这里有个 $ 符号，而 z 不动");
}

// ------------------------------------------------------------ 3. 不该动的
function safetyTests(): void {
	// 英文散文：没有中文锚点
	keepCheck("英文散文里的字母", "the value x is unknown");
	keepCheck("英文句子里的 a", "a matrix is defined as a function");
	keepCheck("英文书名", "- [ ] strang《introduction to linear algebra》");
	keepCheck("专有名词", "将矩阵变换为 Jordan 标准型");

	// 缩写、路径、型号、命名约定
	keepCheck("缩写 e.g.", "例如 e.g. 矩阵，i.e. 向量");
	keepCheck("盘符与路径", "文件在 C:\\data\\note.md 里");
	keepCheck("扩展名", "入口是 main.ts 与 p.js");
	keepCheck("型号 A4", "用 A4 纸打印");
	// 数学符号 1：「加减乘除等于等运算符号和大于小于等状态符号前后都要加空格
	// （如果不是数学语境就不加，比如快捷键 ctrl+c）」—— 运算符没各留一格的一律不认：
	// 那可能是作者故意写的编号 / 连字符 / 命令，也可能就是他在正文里省了空格。排版不猜。
	keepCheck("紧贴的算式不认：x=0", "取 x=0 的值");
	keepCheck("紧贴的算式不认：a+b+c / x>0 / a=0", "流水线 A+B+C 里的 AI 模块与 ab 都算，若 x>0 则正定，取 a=0 与 b=1 的值");
	keepCheck("紧贴的算式不认：单位里的 5/10mm", "把 5/10mm 偏移纳入训练");
	keepCheck("紧贴的数学记号不认：P(A|B)", "条件概率 P(A|B) 的写法");
	caseCheck("留了空格的数学记号照旧：P(A | B)", "条件概率 P(A | B) 的写法", "条件概率 $P(A | B)$ 的写法");
	// `-` 紧贴字母 / 数字时是连字符（通用符号 7），本来就不该当算式
	keepCheck("连字符的型号：仙女座星云 A-7行星", "仙女座星云 A-7行星中，A-7被识别成公式了");
	keepCheck("连字符的型号：F-22 / K-12 / T-34 / AK-47", "F-22 战斗机与 K-12 教育与 T-34 坦克与 AK-47 步枪");
	keepCheck("连字符的算式：A-B / x-1 / n-1", "矩阵 A-B 的乘积，记作 x-1 与 n-1 阶子式");
	keepCheck("希腊字母旁的紧贴运算符也不认", "取 λ-1 的值");
	keepCheck("一半空格的命令写法：cd /d", "同时切换盘和目录：cd /d 路径");
	keepCheck("一半空格的命令与开关：py -0p / git rm -r", "查看解释器 py -0p 与 git rm -r 的用法");
	keepCheck("一半空格的算式不认：x -1 / a =0 / x >0", "取 x -1 的值，有 a =0 成立，若 x >0 则正定");
	keepCheck("日期与范围本来就没有字母", "2024-01-05 与 1-2 个");
	// 规范形态（运算符两侧各留一格）照旧包；正负号是修饰符号（数学符号 3），不算紧贴。
	caseCheck("规范形态照旧：都留一格", "取 x - 1 的值，有 a = 0 成立，若 x > 0 则正定", "取 $x - 1$ 的值，有 $a = 0$ 成立，若 $x > 0$ 则正定");
	caseCheck("前缀正负号不算紧贴", "若 x = -1 则负定", "若 $x = -1$ 则负定");
	keepCheck("命名约定 Q_inv", "记作 Q_inv 与 x^2 与 a_ij 与 Δ_i > 0");
	keepCheck("专有名词后缀", "C 语言 / D 盘 / A 股 / B 站 / X 光 / O 型血");

	// 全库实测逼出来的几条：只按"紧挨中文"判定时这些全都会中招
	keepCheck("两字母缩写不是变量", "由 AI 组装，用 QQ 发送，pg 01 与 tv 都要加空格");
	keepCheck("占位命名 xx", "写 xx 条件而不是判据，xx 原则适用于任何领域");
	// 标题标记的序号不是变量：`附录A` 里的 A 是标记的一部分（文字格式 / 中文 1，
	// 那一格由空格排版补），`第一章矩阵` 里的内容也不该被拆开
	keepCheck("标题标记：附录A 的序号", "附录A矩阵的证明");
	keepCheck("标题标记：附录A 与已有公式", "附录A矩阵中的$A$与$B$");
	keepCheck("标题标记：第一章", "第一章矩阵");
	// 但标题**内容**照常：`第一章矩阵A` 里的 A 是变量，该包
	caseCheck("标题内容照常包公式", "第一章矩阵A", "第一章矩阵$A$");
	keepCheck("分条标签 (a)(b)", "**(a) 分块对角**；**(b) 传感器分布**");
	keepCheck("任务复选框", ["- [x] 已完成", "\t- [ ] 未完成"].join("\n"));
	keepCheck("书名号内部", "如《新 吊带袜天使》《a子计划》等");
	keepCheck("列表标记", ["a. 第一项", "b) 第二项", "1. 第三项", "- 第四项"].join("\n"));
	caseCheck("表格单元格里照常包", ["| 矩阵 A | 向量 b |", "| --- | --- |"].join("\n"), ["| 矩阵 $A$ | 向量 $b$ |", "| --- | --- |"].join("\n"));

	// 保护区
	keepCheck("frontmatter", ["---", "title: 矩阵 A", "---"].join("\n"));
	keepCheck("围栏代码块", ["```sh", "A=1 # 矩阵 A", "```"].join("\n"));
	keepCheck("缩进代码块", ["正文", "", "    A = 1 与 x = 0", ""].join("\n"));
	keepCheck("行内代码", "用 `A = 1` 与 `n 维` 表示");
	keepCheck("双链与链接", "见 [[矩阵 A]] 与 [向量 b](https://a.com/x y)");
	caseCheck("URL 本身与周围单字母都不动", "参考 https://a.com/A=1 与 b 的关系", "参考 https://a.com/A=1 与 b 的关系");
	// 没有 `//` 的 scheme（磁力链接）与裸域名、文件名整段当保护区
	keepCheck("磁力链接", "下载 magnet:?xt=urn:btih:7947d6cdb83a537fc29e9032d2cd2660eacbea25&dn=Queen&xl=3353074364 备用");
	keepCheck("裸域名与文件名", "见 www.bilibili.com/video/BV1xx 与 main.ts 与 file.txt");
	keepCheck("标签", "#矩阵A 与 #线性代数 内容");
	keepCheck("%%注释%%", "%%矩阵 A 与 x = 0%%");
	caseCheck("HTML 标签属性不动，正文照常", '<div class="A" data-x="1">矩阵 A</div>', '<div class="A" data-x="1">矩阵 $A$</div>');
	keepCheck("已有行内公式", "设 $A$ 是 $n$ 阶方阵，$V(F)$ 与 $x = 0$");
	keepCheck("已有公式里没出现过的字母仍然不动", "公式：$$V(x) = {x^T}Px$$ 与后面的 A");
	caseCheck("已有公式里出现过的字母跟着走", "公式：$$V(x) = {x^T}Px$$ 与后面的 x", "公式：$$V(x) = {x^T}Px$$ 与后面的 $x$");
	caseCheck(
		"跨行公式块：内部不动，首行前缀照常",
		["设 n 个变量：$$V(x) = a \\\\", "\tp_{11} & p_{12} \\\\", "\t\\end{bmatrix}$$若 A 可逆"].join("\n"),
		["设 $n$ 个变量：$$V(x) = a \\\\", "\tp_{11} & p_{12} \\\\", "\t\\end{bmatrix}$$若 A 可逆"].join("\n")
	);

	// 开关
	check(
		"关闭后一个字都不改",
		wrapPlainMath("矩阵 A 与 n 维与 V(F) 与 λ", { wrapSymbols: false }),
		"矩阵 A 与 n 维与 V(F) 与 λ"
	);
}

// ------------------------------------------------------------ 4. 幂等
function idempotencyTests(): void {
	const inputs: string[] = [
		"矩阵 A 中最高阶非零子式的阶数，称为 A 的秩",
		"设矩阵A可逆，且 x = 0时恒有 V(x) = 0",
		"线性空间 V(F) 中的向量组称为 V(F) 的一个基，记为 $dim\\,V(F) = n$ 也称 V(F) 为 n 维线性空间",
		"特征值 λ 与 Λ 与 ε > 0 与 a, b ∈ F",
		"- [ ] strang《introduction to linear algebra》 与 C:\\data\\note.md",
		["---", "title: 矩阵 A 与 x = 0", "---", "", "```", "A = 1 与 n 维", "```"].join("\n"),
		["设 n 个变量：$$V(x) = a \\\\", "\tp_{11} & p_{12} \\\\", "\t\\end{bmatrix}$$若 A 可逆"].join("\n"),
		// 变量表要迭代到不动点才收敛：这几条第一次跑会连包两三轮
		"1. x = Tz：x 为原状态，z 为新状态",
		"$e^{At}$ 是矩阵指数，其中 A 是矩阵，t 是时间，e 是常数",
		"设 $z$ 为复数。\n讨论 z 的模长，再讨论 w 的模长，最后讨论 v 的模长",
		"参数 A 满足条件\n因此 A 可逆\n于是 A 的秩为 n",
	];

	for (const input of inputs) {
		const once = wrapPlainMath(input, DEFAULT_TEXT_MATH_OPTIONS);
		const twice = wrapPlainMath(once, DEFAULT_TEXT_MATH_OPTIONS);
		checkTrue("幂等失败", once === twice, `  输入 ${show(input)}\n  一次 ${show(once)}\n  二次 ${show(twice)}`);
	}
	console.log(`智能公式幂等：${inputs.length} 个用例`);
}

// -------------------------------------------------------------------- 运行
console.log("=== 1. 该包的 ===");
goldenTests();

console.log("=== 2. 变量表 ===");
variableTableTests();

console.log("=== 3. 不该动的 ===");
safetyTests();

console.log("=== 4. 幂等 ===");
idempotencyTests();

console.log(`\n共 ${checks} 次检查，失败 ${failures.length} 项`);
for (const message of failures.slice(0, 10)) {
	console.log("\n❌ " + message);
}
if (failures.length > 10) {
	console.log(`\n…… 其余 ${failures.length - 10} 项失败已省略`);
}
if (failures.length > 0) {
	process.exitCode = 1;
}
