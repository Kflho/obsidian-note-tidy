...
function punctuationTests(): void {
	...
	// 真正要留空的是列表里的每个标点自己 —— `, . ! ? :` 后面各一格（英文符号 1）
	caseCheck(
		"全角化：罗列标点自身时不换（紧贴写法）",
		"1. ,/. /!/ /? /:：后面有空格",
		"1. , /. /! / /? /: ：后面有空格"
...
function symbolTests(): void {
	...
	// 标点符号·概论 4「符号与符号之间」：要空格的一方赢
	caseCheck("符号之间：逗号与全角冒号（规则原文的例子）", "如, ：后面有空格", "如, ：后面有空格");
	caseCheck("符号之间：半角冒号与全角冒号", ":：后面有空格", ": ：后面有空格");
	caseCheck("符号之间：半角冒号已空一格", ": ：后面有空格", ": ：后面有空格");
	// 内容与全角标点之间仍然不留空格（规则只在"符号挨着符号"时生效）
	caseCheck("符号之间：内容与全角冒号仍贴紧", "中文 ：内容", "中文：内容");
	caseCheck("符号之间：公式与全角冒号仍贴紧", "$A$ ：说明", "$A$：说明");

	// latex 符号格式 4：`|` 单独一个左右要加空格；5：`&` 前后加空格；4：`→` 前后加空格
	caseCheck("符号：单独一个竖线左右加空格", "4. 运算符号\n\t|：单独一个 | 左右要加空格", "4. 运算符号\n\t| ：单独一个 | 左右要加空格");
	caseCheck("符号：已经空开的竖线保持一格", "4. 运算符号\n\t| ：单独一个 | 左右要加空格", "4. 运算符号\n\t| ：单独一个 | 左右要加空格");
	caseCheck("符号：正文里的竖线分隔符", "$0.00$ |$0.30$", "$0.00$ | $0.30$");
	caseCheck("符号：箭头与前后内容", "→：和前后内容间要加空格", "→ ：和前后内容间要加空格");
	// 规则只回答"左边要不要一格、右边要不要一格"：`&` 在行首，左边自然没有东西可空，
	// 右边照它的规则留一格
	caseCheck("符号：和号在行首（左边没有东西）", "&：前后有内容都要加排版空格", "& ：前后有内容都要加排版空格");
	caseCheck("符号：和号在正文里", "甲&乙", "甲 & 乙");
	caseCheck("符号：和号在行尾（右边没有东西）", "7. 并列：&", "7. 并列： &");
	...
	// 半角引号也是包裹符号：**内侧不留空格**、外侧不动（204 行「突出里面内容」）
	caseCheck("包裹：半角引号内侧收空格", '他说" 你好 "了', '他说"你好"了');
	caseCheck("包裹：半角引号里的符号贴紧", '写作 " + " 与 "+" 两种', '写作 "+" 与 "+" 两种');
	caseCheck("包裹：半角引号外侧不动", '他说 "你好" 了', '他说 "你好" 了');
	caseCheck("包裹：落单的引号不动", '2" 的管子', '2" 的管子');
	caseCheck("包裹：英文引文照旧", 'He said "hello" loudly', 'He said "hello" loudly');
